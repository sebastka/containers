...
//
// It guards against the "Log in goroutine after Test completed" panic itself instead of relying on
// the library to join its log-pump goroutine. On testcontainers-go v0.42.0, stopLogProduction only
// cancels the pump's context and returns without waiting, so a late line can still arrive after the
// test has finished. A t.Cleanup marks the consumer closed and Accept drops anything after that.
...
func newTLogConsumer(t *testing.T) *tLogConsumer {
	t.Helper()
...
func runContainer(t *testing.T, ctx context.Context, image string, opts ...testcontainers.ContainerCustomizer) testcontainers.Container {
	...
	opts = append([]testcontainers.ContainerCustomizer{
		...
		testcontainers.WithLogConsumers(newTLogConsumer(t)),
	}, opts...)
